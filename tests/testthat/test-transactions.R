...
test_that("add_transactions works", {
  expect_equal(
    sum(withdrawals$trx_amt),
    sum(expo_trx$trx_amt_Base) +
      sum(expo_trx$trx_amt_Rider)
  )
  expect_equal(
    nrow(withdrawals),
    sum(expo_trx$trx_n_Base) +
...
test_that("Type checks work", {
  expect_error(
    add_transactions(as.data.frame(expo), withdrawals),
    regexp = 'must be an `exposed_df` object'
  )
  expect_error(
    add_transactions(1, withdrawals),
    regexp = 'must be an `exposed_df` object'
  )
...
test_that("Renaming works and name conflicts work", {
  expect_error(
    add_transactions(expo |> head(), withdrawals4),
    regexp = "Can't rename"
  )
  expect_no_error(add_transactions(
    expo,
    withdrawals4,
    col_pol_num = "a",
    col_trx_date = "b",
  ...
  expect_error(
    add_transactions(expo_trx, withdrawals),
...
test_that("exposed_df persists after adding transactions", {
  ...
  expect_s3_class(
    add_transactions(expo |> group_by(pol_yr), withdrawals),
    "exposed_df"
  )
})
...
test_that("Date format checks work", {
  ...
  expect_error(
    add_transactions(expo, withdrawals5),
