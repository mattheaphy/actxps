...
`exp_shiny()` is a powerful function that launches an interactive Shiny app containing filters, grouping variables, plots produced by `autoplot()`, tables produced by `autotable()`, and a data export button. This function requires an `exposed_df` object^[See `expose()` or `as_exposed_df()` for more information].
