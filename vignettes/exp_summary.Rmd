...
As common metric in experience studies is actual-to-expected, or A/E ratio.
...
Let's assume we have two sets of expected rates. The first set is a vector that varies by policy year. The second set is either 1.5% or 3.0% depending on whether the policy has a guaranteed income benefit. First, we need to attach these assumptions to our exposure data, and we will use the names `expected_1` and `expected_1`. Then we pass these names to the `expected` argument when we call `exp_stats()`.
...
If not additional arguments are passed, `summary()` returns a single row of aggregate results.
